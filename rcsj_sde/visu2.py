...
from rcsj_sde.utils2 import linear_reference, ambegaokar_overdamped
...
def plotFFTmap(simu: SimuResult,
    ...
    V_start_idx = jnp.min(jnp.where(simu.V > V_th)) + 1
    xf_start_idx = 0
    ...
    if vmin is None:
        vmin=jnp.min(simu.psd_dbm[:])
    
    if vmax is None:
        vmax=jnp.max(simu.psd_dbm[:])
...
def overlay_cuts(simus: Sequence[SimuResult],
    ...
    colors = plt.cm.coolwarm(jnp.linspace(1,0,len(simus_filtered)))
    ...
    for i, simu in enumerate(simus_filtered):
        ...
        if V_cut is None:
            ...
            V_idx = jnp.argmin(jnp.abs(simu.V - V_cut))
