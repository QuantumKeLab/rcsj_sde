...
from rcsj_sde.rcsj2 import rcsj_solver
from rcsj_sde.simuresult2 import SimuResult
from rcsj_sde.utils2 import hbar_over_2e
...
def simulation_voltage(jj: JosephsonJunction,
                       ...
                       progressbar: bool = True) -> jnp.ndarray:
    """
    ...
    for (k, i_dc) in enumerate(tqdm(i_dc_range, position=0, leave=True, disable=not progressbar)):
        ...
        V[k] = hbar_over_2e*jnp.mean(sol[:, 1])/jj.t_c

        # carry over the final phi, phidot as the initial condition for the next bias current
        reduced_phi = sol[-1, 0] % (2*jnp.pi)
        y0 = jnp.asarray([reduced_phi, sol[-1, 1]], dtype=y0.dtype)
...
def simulation_full(jj: JosephsonJunction,
    ...
    # reserve storage for output 
    sol_stored = jnp.zeros((I_points, tau_points, 2)) # last axis dim: 2, for phi and phidot

    # loop over current bias values
    for (k, i_dc) in enumerate(tqdm(i_dc_range, position=0, leave=True, disable=not progressbar)):
        key = jax.random.PRNGKey(0)
        sol = rcsj_solver(key, jj.epsilon, jj.beta, i_dc, jj.a, jj.b, y0, tau, i_ac, f_ac)
