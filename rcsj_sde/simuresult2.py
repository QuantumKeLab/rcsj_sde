...
from rcsj_sde.utils2 import hbar_over_2e, v2_to_dbm
from rcsj_sde.junction2 import JosephsonJunction
...
class SimuResult():
    ...
    I_DC_range: (N_I,) jnp.ndarray
        DC current points over which the simulation was carried out.
    tau : (N_tau,) jnp.ndarray
        Normalized time points
    sol_stored : (N_I, N_tau, 2) jnp.ndarray
    ...
    tau: jnp.ndarray
    sol_stored: jnp.ndarray
    cutoff_threshold_idx: Optional[int] = 0
    
    def __post_init__(self):
        
        self.tau_max = int(jnp.max(self.tau))
    ...
    def save(self, fname: str) -> None:
        ...
        jnp.savez_compressed(fname,
                            jj=jnp.array([self.jj.to_json()]),
                            I_DC_range=self.I_DC_range,
                            tau=self.tau,
                            sol_stored=self.sol_stored,
                            cutoff_threshold_idx=jnp.array([self.cutoff_threshold_idx]),
                            )
    ...
    def export(self, fname: str) -> None:
        ...
        jnp.savetxt(fname + "_VI.txt",
                   jnp.vstack([self.I_DC_range, self.V]).T,
                   delimiter=delimiter,
                   header=header_VI)
        
        jnp.savetxt(fname + "_PSD_dB.txt", self.psd_dbm, delimiter=delimiter, header=header)
        jnp.savetxt(fname + "_PSD.txt", self.psd, delimiter=delimiter, header=header)
        jnp.savetxt(fname + "_f.txt", self.xf, delimiter=delimiter, header=header)
